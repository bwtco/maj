...
class HrPayslip(models.Model):
    ...
    def _compute_worked_days_line_ids(self):
        ...
        bonus_line_ids = bonus_line_obj.search([('employee_id', '=', self.employee_id.id), ('state', 'in', ['done'])])
        for line in bonus_line_ids:
            ...
            for period in line.period_ids:
                if period.date_start == self.date_from or period.date_stop == self.date_to:
                    ...
                    lines_to_keep = self.input_line_ids.filtered(lambda x: x.input_type_id != bonus_type)
