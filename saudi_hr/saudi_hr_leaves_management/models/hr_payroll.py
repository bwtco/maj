...
class HrPayslip(models.Model):
    ...
    #         result = self._get_worked_day_lines()
    ...
    def _get_worked_day_lines(self):
        ...
        sick_leaves = self.env.ref('hr_holidays.holiday_status_sl')
        maternity_leaves = self.env.ref('saudi_hr_leaves_management.holiday_status_maternity')
        holiday_ids = self.env['hr.leave'].search([('employee_id', '=', self.employee_id.id),
                                                   ('state', '=', 'validate'),
                                                   ('holiday_status_id', '=', sick_leaves.id),
                                                   '|', '&',
        ...
        leave_details = self.env['leave.detail'].search([
                    ...
                    ('period_id.date_start', '<=', self.date_to),
                    ('period_id.date_stop', '>=', self.date_from),
                ])
        for line in res:
            if sick_leaves.work_entry_type_id.id == line.get('work_entry_type_id'):
                res[0]['number_of_days'] = sum(leave_details.mapped('paid_leave'))
                res[0]['number_of_hours'] = sum(leave_details.mapped('leave_hours'))
        if sum(leave_details.mapped('unpaid_leave')) > 0:
            res.append({'work_entry_type_id':  self.env.ref('saudi_hr_leaves_management.work_entry_type_sickleave_unpaid').id,
                        'name': 'Sick Time Off Unpaid',
                        'number_of_days': sum(leave_details.mapped('unpaid_leave')),
                        'number_of_hours': sum(leave_details.mapped('unpaid_leave_hours')),
                        'amount': (self.contract_id.wage /30) * sum(leave_details.mapped('unpaid_leave')),
                        })
        # Maternity leave paid and unpaid
        holiday_ids = self.env['hr.leave'].search([('employee_id', '=', self.employee_id.id),
                                                   ('state', '=', 'validate'),
                                                   ('holiday_status_id', '=', maternity_leaves.id),
                                                   '|', '&',
                                                   ('date_from', '>=', self.date_from),
                                                   ('date_from', '<=', self.date_to),
                                                   '&',
        ...
        for line in res:
            if maternity_leaves.work_entry_type_id.id == line.get('work_entry_type_id'):
                line['number_of_days'] = sum(leave_details.mapped('paid_leave'))
                line['number_of_hours'] = sum(leave_details.mapped('leave_hours'))
        if sum(leave_details.mapped('unpaid_leave')) > 0:
            res.append({'work_entry_type_id':  self.env.ref('saudi_hr_leaves_management.work_entry_type_maternity_leave_unpaid').id,
                        'name': 'Maternity Time Off Unpaid',
                        'number_of_days': sum(leave_details.mapped('unpaid_leave')),
                        'number_of_hours': sum(leave_details.mapped('unpaid_leave_hours')),
                        'amount': (self.contract_id.wage /30) * sum(leave_details.mapped('unpaid_leave')),
                        })
