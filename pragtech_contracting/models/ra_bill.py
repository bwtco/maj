...
class RABill(models.Model):
    ...
    final_total_payable = fields.Float(
        'Total Payable', help='This is total payable incuding debit,credit,advance', compute='get_total_payable_amt',
        store=True)
    ...
    retention_amount = fields.Float('Retention', help="Retention amount of workorder=(retn *100)/untaxed amt",
                                    compute='get_wo_retension_amt')
    ...
    def get_total_payable_amt(self):
        ...
        for this in self:
            ...
            this.final_total_payable = total - retention_amount - \
    ...
    def get_advance_recovery(self, partner, wizard_id, workorder):
        ...
        trascations = self.env['transaction.transaction'].search(
            [('transaction_type', '=', 'advance'), ('project_id', '=', self.project_id.id),
             ('stage_id', '=', st_id.id)])
    ...
    def get_credit_note(self, partner, wizard_id, workorder):
        ...
        st_id = self.env['stage.master'].search([('approved', '=', True)])
        trascations = self.env['transaction.transaction'].search(
            [('transaction_type', '=', 'credit_note'), ('project_id', '=', self.project_id.id),
             ('stage_id', '=', st_id.id)])
        recovered_till_date = 0
        for line in trascations:
            if line.balance_amount > 0:
    ...
    def get_debit_note(self, partner, wizard_id, workorder):
        ...
        st_id = self.env['stage.master'].search([('approved', '=', True)])
        trascations = self.env['transaction.transaction'].search(
            [('transaction_type', '=', 'debit_note'), ('project_id', '=', self.project_id.id),
             ('stage_id', '=', st_id.id)])
        for line in trascations:
            if line.balance_amount > 0:
        ...
        for wo_line in workorder.order_line:
            for task_line in wo_line.payment_schedule_line_ids:
                compl_obj = self.env['work.completion'].search([('task_id', '=', task_line.task_id.id), (
                    'total_percent', '>', 0), ('workorder_id', '=', self.workorder_id.id),
                ...
                for completed_task in compl_obj:
                    for line in completed_task.order_line:
                        if  line.bill and line.stage_id == st_id:
                            ...
                            payble_amt = self.payble_amount(completed_task.workorder_id.id, wo_line,
                                                            completed_task.task_id.id,
                                                            completed_task.total_percent, amt_to_release_percent,
                                                            wo_line.basic_amount)
                            dict = [0,0,{'work_completion_sequence': completed_task.id,
    ...
    def create(self, vals):
        ...
        for credit in self.credit_note_ids:
            ...
            #                 credit_obj.recovered_till_date=credit_obj.recovered_till_date+credit.this_bill_recovery
                ...
                raise UserError(
                    _('This Bill recovery cannot be greater then balance amount'))
        if self.final_total_payable < 0 and self.final_total_payable < (
                self.wo_balance_advance + self.wo_balance_debit + self.till_date_billed):
            ...
            if context.get('copy') == True:
                ...
                for bill_line in self.rabill_line_ids:
                    completion_line = self.env['work.completion.line'].search(
                        [('order_id', '=', bill_line.work_completion_sequence.id),
                         ('sequence', '=', bill_line.work_completion_line_sequence)])
    ...
    def create_ra_bill_invoice(self):
        ...
        invoice_obj = invoice_obj.create({'name': 'RA BILL' + ' ' + str(self.name),
        ...
        for line in self.rabill_line_ids:
            ...
            invoice_obj.invoice_line_ids.with_context(check_move_validity=False).create(
                [acct_line_credit, acct_line_debit])
